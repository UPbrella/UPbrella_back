...
public class RentService {
    ...
    public ReturnFormResponse findReturnForm(long storeId, User userToReturn, String salt,
        String signature) {
        ...
        History history = rentRepository.findByUserIdAndReturnedAtIsNull(userToReturn.getId())
            .orElseThrow(
                () -> new NonExistingUmbrellaForRentException("[ERROR] 해당 유저가 대여 중인 우산이 없습니다."));
    ...
    public void addRental(RentUmbrellaByUserRequest rentUmbrellaByUserRequest, User userToRent) {
        ...
        Umbrella willRentUmbrella = umbrellaService.findUmbrellaById(
            rentUmbrellaByUserRequest.getUmbrellaId());
        ...
        StoreMeta rentalStore = storeMetaService.findStoreMetaById(
            rentUmbrellaByUserRequest.getStoreId());
        ...
        ConditionReport conditionReportToSave = new ConditionReport(
            history, conditionReport, null, null);
    ...
    public RentalHistoriesPageResponse findAllHistories(HistoryFilterRequest filter,
        ...
        return RentalHistoriesPageResponse.of(findAllRentalHistory(filter, pageable),
            countOfAllHistories, countOfAllPages);
    }
    ...
    private void addImprovementReportFromReturnByUser(History history,
        ReturnUmbrellaByUserRequest request) {

        if (request.getImprovementReportContent() == null) {
            return;
        }

        improvementReportService.addImprovementReportFromReturn(history,
            request.getImprovementReportContent());
    }
    ...
    public History findRentalHistoryByUser(SessionUser sessionUser) {
        ...
        return rentRepository.findByUserIdAndReturnedAtIsNull(sessionUser.getId())
            .orElseThrow(
                () -> new NonExistingBorrowedHistoryException("[ERROR] 사용자가 빌린 우산이 없습니다."));
    }
